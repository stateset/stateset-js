...
export class ComputeQuerier {
  private readonly grpc: import("../protobuf_stuff/stateset/compute/v1beta1/query").GrpcWebImpl;
  private encryption?: EncryptionUtils;
  private client?: import("../protobuf_stuff/stateset/compute/v1beta1/query").QueryClientImpl;
  ...
  private async init() {
    if (!this.client) {
      this.client = new (
        await import("../protobuf_stuff/stateset/compute/v1beta1/query")
    ...
    if (!this.encryption) {
      this.encryption = new EncryptionUtilsImpl(
        new (
          await import("../protobuf_stuff/stateset/registration/v1beta1/query")
        ).QueryClientImpl(this.grpc),
      );
    }
  }
...
export function bytesToAddress(
  bytes: Uint8Array,
  prefix: string = "stateset",
): string {
  return bech32.encode(prefix, bech32.toWords(bytes));
}

function contractInfoFromProtobuf(
  contractInfo: import("../protobuf_stuff/stateset/compute/v1beta1/types").ContractInfo,
): ContractInfo {
  return {
...
function codeInfoResponseFromProtobuf(
  codeInfo?: import("../protobuf_stuff/stateset/compute/v1beta1/query").CodeInfoResponse,
): CodeInfoResponse {
  return codeInfo
    ? {
