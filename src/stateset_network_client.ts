...
export type TxOptions = {
   ...
   *
   * Smaller intervals will cause more load on your node provider. Keep in mind that blocks on Stateset Network take about 6 seconds to finalize.
   *
   ...
   * This is useful when using {@link BroadcastMode.Async} or when you don't want statesetjs
...
export type TxSender = {
  /**
   * Sign and broadcast a transaction to Stateset Network.
   *
...
export class StatesetNetworkClient {
  ...
  /** Creates a new StatesetNetworkClient client. For a readonly client pass just the `grpcUrl` param. */
  public static async create(
    options: CreateClientOptions,
  ): Promise<StatesetNetworkClient> {
    const { GrpcWebImpl } = await import(
      "./protobuf_stuff/stateset/compute/v1beta1/query"
    );
    let grpcWeb: import("./protobuf_stuff/stateset/compute/v1beta1/query").GrpcWebImpl;
    ...
    return new StatesetNetworkClient(
      grpcWeb,
      txService,
      query,
      msgDecoderRegistry,
  ...
  private constructor(
    grpc: import("./protobuf_stuff/stateset/compute/v1beta1/query").GrpcWebImpl,
    ...
    return await Promise.all(
      txResponses.map(async (tx, i) => {
        ...
        for (let i = 0; i < decodedTx.body!.messages.length; i++) {
          ...
          if (
            msg.typeUrl === "/stateset.compute.v1beta1.MsgInstantiateContract"
          ) {
            contractInputMsgFieldName = "initMsg";
          } else if (
            msg.typeUrl === "/stateset.compute.v1beta1.MsgExecuteContract"
          ) {
...
function extractNonce(msg: ProtoMsg): Uint8Array {
  if (msg.typeUrl === "/stateset.compute.v1beta1.MsgInstantiateContract") {
    return msg.value.initMsg.slice(0, 32);
  }
  if (msg.typeUrl === "/stateset.compute.v1beta1.MsgExecuteContract") {
